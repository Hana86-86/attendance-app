  ...
  <table class="table att-table">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td class="mono">{{ $att->clock_in?->format('H:i')  ?? 'ー' }}</td>
        <td class="mono">{{ $att->clock_out?->format('H:i') ?? 'ー' }}</td>

        {{-- 休憩時間：0:00 または null のときは「ー」 --}}
        <td class="mono">
          {{ (!empty($att?->break_hm) && $att->break_hm !== '0:00') ? $att->break_hm : 'ー' }}
        </td>

        {{-- 合計時間：0:00 または null のときは「ー」 --}}
        <td class="mono">
          {{ (!empty($att?->work_hm) && $att->work_hm !== '0:00') ? $att->work_hm : 'ー' }}
        </td>
